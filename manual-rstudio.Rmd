...
output: 
  html_document:
    ...
    toc_float: true
    number_section: true
date:  "Written, maintained and updated by VLIZ - Version 1 (`r format(Sys.time(), '%d/%m/%Y')`)"
---
...
The ScheldeMonitor environment can be accessed at (https://rstudio.scheldemonitor.org/auth-sign-in) using your received credentials:
...
2. The toolbox offers several criteria to filter the database of ScheldeMonitor. These criteria differ for biotic and abiotic data, yet are not mandatory to be selected. When criteria are selected, the counter on the right side of the screen shows the remaining number of records that match the chosen criteria.
...
```{r echo=FALSE, out.width='80%'}
knitr::include_graphics("./images_manual_rstudio/image18_scheldemonitor_data_summary.png")
```
...
```{r echo=FALSE, out.width='80%'}
knitr::include_graphics("./images_manual_rstudio/image21_scheldemonitor_data_download_progress.png")
```
