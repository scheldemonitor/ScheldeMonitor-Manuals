...
It is also possible to link the RStudio environment with an existing project from the ScheldeMonitor GitHub organization. How to work with GitHub in relation with RStudio, is discussed in an additional GitHub [manual](https://www.scheldemonitor.org/sites/scheldemonitor.be/files/2020-12/manual-github.pdf).
...
New users should first register to receive their personal credentials. This can be done using this [link](https://rshiny.lifewatch.be/account?p=register) or by contacting the [helpdesk](#helpdesk) of ScheldeMonitor stating a reason to use the RStudio environment.
...
You can also change the working directory by selecting the “Session” menu and “Set Working Directory”.
...
observeEvent(input$Map_click, {
  ...
  leafletProxy('Map') %>%
    ...
    addMarkers(lng = input$Map_click$lng,
               ...
               popup = paste("Longitude=", round(input$Map_click$lng, 2), "and", "Latitude=", round(input$Map_click$lat, 2)))
})
```
